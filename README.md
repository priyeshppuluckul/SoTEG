...
where $\varepsilon_{sky}$ is the  emissivity of the sky which depends on dew temperature $T_{d}$ and sky opacity N. Models in [SkyTemperature.py](./SkyTemperature.py) can be used if dew point and opaque sky cover are known. For approximate calculations, the temperature of the sky can be assumed to be 0°C.
